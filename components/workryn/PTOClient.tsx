...
interface PTOClientProps { currentUser: { id: string; name: string; role: string; avatarColor: string }; types: PtoType[]; balances: Balance[]; initialRequests: PtoRequest[]; allUsers: UserInfo[]; pendingCount: number; intuitMappings: IntuitMap[]; isElevated: boolean; intuitConnected?: boolean; intuitCompanyName?: string | null }

export default function PTOClient({ currentUser, types, balances, initialRequests, allUsers, pendingCount: initialPendingCount, intuitMappings: initialMappings, isElevated, intuitConnected: initialIntuitConnected = false, intuitCompanyName: initialCompanyName = null }: PTOClientProps) {
  ...
  return (
    <div className="w-pto-page">
      {oauthToast && (
        <div className={`w-pto-toast ${oauthToast.type}`} role="alert">
          {oauthToast.type === 'success' ? <Check size={16} /> : <AlertTriangle size={16} />}
          <span>{oauthToast.message}</span>
          <button className="w-pto-toast-close" onClick={() => setOauthToast(null)}><X size={14} /></button>
        </div>
      )}
      ...
      {tab==='intuit'&&isElevated&&<div className="w-pto-intuit">
        <div className={`w-pto-intuit-connection ${intuitConnected ? 'connected' : 'disconnected'}`}>
          {intuitConnected ? (<><div className="w-pto-intuit-connection-info"><Check size={18} /><div><strong>Connected to QuickBooks</strong>{companyName && <span className="w-pto-intuit-company">{companyName}</span>}</div></div><a href="/api/workryn/pto/intuit/auth" className="w-btn w-btn-sm w-btn-ghost w-focus-ring">Reconnect</a></>) : (<><div className="w-pto-intuit-connection-info"><AlertTriangle size={18} /><div><strong>QuickBooks Not Connected</strong><span className="w-pto-intuit-connect-desc">Connect your QuickBooks Online account to enable payroll sync for PTO requests.</span></div></div><a href="/api/workryn/pto/intuit/auth" className="w-btn w-btn-primary w-focus-ring"><Link2 size={16} /> Connect to QuickBooks</a></>)}
        </div>
        <div className="w-pto-intuit-header"><div><h3>Intuit QuickBooks Employee Mapping</h3><p className="w-pto-intuit-desc">Map Workryn users to QuickBooks employees for automatic PTO payroll sync. Auto-matching uses email first, then name.</p></div><button className="w-btn w-btn-primary w-focus-ring" onClick={syncIntuitEmployees} disabled={syncing || !intuitConnected}><RefreshCw size={16} className={syncing?'w-spin':''}/>{syncing?'Syncing...':'Sync Employees'}</button></div>{syncResults&&<div className="w-pto-sync-results">{syncResults.error?<div className="w-pto-sync-error"><AlertTriangle size={16}/> {syncResults.error}{syncResults.detail&&<pre>{syncResults.detail}</pre>}</div>:<><div className="w-pto-sync-stat-grid"><div className="w-pto-sync-stat matched"><span className="w-pto-sync-stat-num">{syncResults.matched?.length||0}</span><span>Newly Matched</span></div><div className="w-pto-sync-stat existing"><span className="w-pto-sync-stat-num">{syncResults.already_mapped||0}</span><span>Already Mapped</span></div><div className="w-pto-sync-stat unmatched"><span className="w-pto-sync-stat-num">{syncResults.unmatched_qbo?.length||0}</span><span>Unmatched (QBO)</span></div><div className="w-pto-sync-stat unmatched"><span className="w-pto-sync-stat-num">{syncResults.unmatched_workryn?.length||0}</span><span>Unmatched (Workryn)</span></div></div>{syncResults.matched?.length>0&&<div className="w-pto-sync-match-list"><h4>Auto-Matched Employees</h4>{syncResults.matched.map((m:any,i:number)=><div key={i} className="w-pto-sync-match-row"><span>{m.workrynUser}</span><span className="w-pto-sync-arrow"></span><span>{m.qboEmployee}</span><span className="w-pto-sync-method">via {m.matchedBy}</span></div>)}</div>}{syncResults.unmatched_qbo?.length>0&&<div className="w-pto-sync-unmatched"><h4>Unmatched QuickBooks Employees (need manual mapping)</h4>{syncResults.unmatched_qbo.map((u:any)=><div key={u.id} className="w-pto-sync-unmatched-row"><span>{u.name}</span>{u.email&&<span className="w-pto-sync-email">{u.email}</span>}<span className="w-pto-sync-id">QBO #{u.id}</span></div>)}</div>}</>}</div>}<div className="w-pto-intuit-mappings"><h4>Current Employee Mappings ({intuitMappings.length})</h4>{intuitMappings.length===0?<div className="w-pto-empty"><Link2 size={24}/><p>{intuitConnected ? 'No employee mappings yet. Click "Sync Employees" to auto-match.' : 'Connect to QuickBooks first, then sync employees.'}</p></div>:<div className="w-pto-mapping-table"><div className="w-pto-mapping-header"><span>Workryn User</span><span>QBO Employee</span><span>QBO Email</span><span>Status</span></div>{intuitMappings.map(m=><div key={m.id} className="w-pto-mapping-row"><span className="w-pto-mapping-user">{m.user.name||m.user.email||'Unknown'}</span><span>{m.intuitDisplayName||`#${m.intuitEmployeeId}`}</span><span className="w-pto-mapping-email">{m.intuitEmail||''}</span><span className={`w-pto-mapping-status ${m.syncStatus.toLowerCase()}`}>{m.syncStatus}</span></div>)}</div>}</div></div>}
