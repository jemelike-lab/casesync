...
import {
  Plus, CheckSquare, Clock, User, X, Search,
  MoreHorizontal, MessageSquare, Loader2, AlertCircle,
} from 'lucide-react'
...
const COLUMNS = [
  { id: 'TODO', label: 'To Do', color: '#64748b', gradient: 'linear-gradient(135deg, #64748b 0%, #94a3b8 100%)' },
  { id: 'IN_PROGRESS', label: 'In Progress', color: '#6366f1', gradient: 'linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)' },
  { id: 'IN_REVIEW', label: 'In Review', color: '#f59e0b', gradient: 'linear-gradient(135deg, #f59e0b 0%, #fbbf24 100%)' },
  { id: 'DONE', label: 'Done', color: '#10b981', gradient: 'linear-gradient(135deg, #10b981 0%, #34d399 100%)' },
]
...
// Floating menu rendered at fixed coords to escape overflow clipping
function FloatingMenu({
  anchor, columns, onMoveTo, onEdit, onDelete, onClose
}: {
  anchor: DOMRect
  ...
  const left = Math.max(8, anchor.right - 180)
  ...
  return (
    ...
    >
      <div className="dropdown-item" style={{ fontWeight: 700, fontSize: '0.6875rem', color: 'var(--text-muted)', pointerEvents: 'none', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
        Move to
      </div>
      {columns.map(col => (
...
export default function TasksClient({ initialTasks, users, departments, currentUserId }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    assignedToId: '', departmentId: '', dueDate: '', tags: '',
  })
  ...
  function openEdit(task: Task) {
    ...
    setForm({
      ...
      priority: task.priority, assignedToId: task.assignedTo?.id ?? '',
    ...
    <>
      <div className="page-header" style={{ padding: '24px 32px 20px' }}>
        <div className="flex items-center justify-between" style={{ marginBottom: 16 }}>
          ...
          <button className="btn btn-gradient focus-ring" onClick={openCreate} id="btn-create-task">
        ...
        <div style={{ background: 'var(--bg-overlay)', borderRadius: 99, height: 5, marginBottom: 16, overflow: 'hidden' }}>
          <div style={{
            ...
            background: 'var(--brand-gradient)', borderRadius: 99,
            transition: 'width 0.5s cubic-bezier(0.4, 0, 0.2, 1)',
            boxShadow: pct > 0 ? '0 0 12px rgba(99,102,241,0.4)' : 'none',
          }} />
        </div>
        {/* Filter bar */}
      ...
      <div className="page-body" style={{ paddingTop: 20 }}>
        <div className="kanban-board">
          {COLUMNS.map((col, colIndex) => {
            ...
            return (
              <div key={col.id} className="kanban-col animate-slide-up" style={{ animationDelay: `${colIndex * 60}ms` }}>
                {/* Gradient accent bar at top of column */}
                ...
                <div className="kanban-cards">
                  ...
                  {colTasks.map((task, i) => (
                    <TaskCard
                      ...
                      isMenuOpen={menuState?.taskId === task.id}
                    />
                  ))}
                </div>
              </div>
          ...
          <div className="modal animate-scale-in" onClick={e => e.stopPropagation()} style={{ maxWidth: 560 }}>
            ...
            <div className="modal-body" style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div className="form-group">
                ...
                <input id="task-title" className="input focus-ring" placeholder="What needs to be done?" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} autoFocus />
              </div>
              <div className="form-group">
...
function TaskCard({ task, onTaskClick, onMenuOpen, isMenuOpen, index }: {
  task: Task
  onTaskClick: () => void
  ...
  return (
    <div className="task-card" onClick={onTaskClick} style={{ animationDelay: `${index * 30}ms` }}>
