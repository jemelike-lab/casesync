...
export async function POST(req: NextRequest) {
  ...
  let body: {
    typeId: string; startDate: string; endDate: string; totalHours: number;
  ...
  if (!typeId || !startDate || !endDate || !totalHours)
    return NextResponse.json({ error: 'typeId, startDate, endDate, and totalHours are required' }, { status: 400 })
  if (totalHours <= 0)
    return NextResponse.json({ error: 'totalHours must be positive' }, { status: 400 })
  ...
  if (!ptoType || !ptoType.isActive)
    return NextResponse.json({ error: 'Invalid PTO type' }, { status: 400 })

  let balance = await db.ptoBalance.findUnique({
    where: { userId_typeId: { userId: user.id, typeId } }
  })
  if (!balance) {
    balance = await db.ptoBalance.create({
      data: { userId: user.id, typeId, accrued: 0, used: 0, pending: 0, adjustment: 0 }
    })
  }

  const available = balance.accrued + balance.adjustment - balance.used - balance.pending
  if (totalHours > available)
    return NextResponse.json({ error: `Insufficient balance. Available: ${available.toFixed(1)} hrs` }, { status: 400 })

  const [request] = await db.$transaction([
    db.ptoRequest.create({
      data: {
        userId: user.id, typeId, startDate: start, endDate: end, totalHours,
        isHalfDay: isHalfDay ?? false, halfDayPeriod: halfDayPeriod ?? null,
    ...
    db.ptoBalance.update({
      where: { userId_typeId: { userId: user.id, typeId } },
      data: { pending: { increment: totalHours } }
    }),
  ])
