...
export async function POST(req: NextRequest) {
  ...
  if (!clientId || !clientSecret)
    return NextResponse.json({ error: 'Intuit credentials not configured. Set INTUIT_CLIENT_ID and INTUIT_CLIENT_SECRET.' }, { status: 500 })
  ...
  if (!code || !realmId || !redirectUri)
    return NextResponse.json({ error: 'code, realmId, and redirectUri are required' }, { status: 400 })
  ...
  try {
    ...
    if (!tokenRes.ok) {
      ...
      return NextResponse.json({ error: 'Failed to exchange authorization code', detail: errText }, { status: 502 })
    }
    tokenData = await tokenRes.json()
  } catch (err) {
    return NextResponse.json({ error: 'Network error contacting Intuit', detail: String(err) }, { status: 502 })
  }
